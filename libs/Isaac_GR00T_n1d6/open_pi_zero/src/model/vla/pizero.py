...
from ACG.libs.Isaac_GR00T_n1d6.open_pi_zero.src.model.kv_cache import KVCache
from ACG.libs.Isaac_GR00T_n1d6.open_pi_zero.src.model.vla.modules import (
    ActionEncoder,
    SinusoidalPosEmb,
)
from ACG.libs.Isaac_GR00T_n1d6.open_pi_zero.src.utils.decorator import NoSyncBase
from ACG.libs.Isaac_GR00T_n1d6.open_pi_zero.src.utils.monitor import log_execution_time
...
class PiZero(nn.Module, NoSyncBase):
    ...
    def infer_actions_naive(
        ...
        inputs_embeds = inputs_embeds.repeat(num_repeats, 1, 1)
        proprio_embeds = proprio_embeds.repeat(num_repeats, 1, 1)
        causal_mask = causal_mask.repeat(num_repeats, 1, 1, 1)
        vlm_position_ids = vlm_position_ids.repeat(num_repeats, 1)
        proprio_position_ids = proprio_position_ids.repeat(num_repeats, 1)
        action_position_ids = action_position_ids.repeat(num_repeats, 1)

        # sample pure action noise
        action = torch.randn((num_repeats, self.horizon_steps, self.action_dim), device=device, dtype=dtype)
        action = action.repeat(2, 1, 1)
