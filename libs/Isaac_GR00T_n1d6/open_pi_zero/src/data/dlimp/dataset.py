...
from ACG.libs.Isaac_GR00T_n1d6.open_pi_zero.src.data.dlimp.utils import parallel_vmap
...
def _decode_example(
    ...
    parsed_tensors = {
        key: (
            tf.io.parse_tensor(parsed_features[key], spec.dtype)
            if spec is not None
...
def _broadcast_metadata(
    ...
    traj_len = tf.reduce_max([tl for tl in traj_lens.values() if tl is not None])
