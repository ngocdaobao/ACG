...
from robomimic.utils.dataset import MetaDataset, R2D2Dataset, SequenceDataset
from tianshou.env import SubprocVectorEnv


def get_exp_dir(config, auto_remove_exp_dir=False, overwrite=False):
    """
...
def dataset_factory(config, obs_keys, filter_by_attribute=None, dataset_path=None, lang_encoder=None):
    ...
    elif config.train.data_format == "dexmg":
        from gr00t.data.dataset_robocasa import DexmgDataset, MetaDexmgDataset
        ds_class = DexmgDataset
        meta_ds_class = MetaDexmgDataset
    else:
...
def run_rollout(
    ...
    if hasattr(policy.policy, 'reset_prior'):
        policy.policy.reset_prior()
    ...
    for step_i in LogUtils.tqdm(range(horizon)):
        ...
        if batched:
            ...
            if ds_format == "dexmg":
                ac = policy(ob=policy_ob, goal=goal_dict, batched=True, env_name=env.get_env_attr("name", id=[0])[0])
            else:
                ac = policy(ob=policy_ob, goal=goal_dict, batched=True)  # , return_ob=True)
        else:
            policy_ob = ob_dict
            if ds_format == "dexmg":
                ac = policy(ob=policy_ob, goal=goal_dict, env_name=env.name)
            else:
            ...
            if video_count % video_skip == 0:
                if batched:
                    ...
                    for env_i in range(len(env)):
                        frame_tens_keys = ['robot0_agentview_left_image', 'agentview_image', 'frontview_image']
                        frame_tens = None
                        for key in frame_tens_keys:
                            if key in policy_ob:
                                frame_tens = policy_ob[key]
                                break
                        if frame_tens is None:
                            raise ValueError("No valid frame tensor found in policy_ob. Expected one of: {}".format(frame_tens_keys))

                        is_single_obs = len(frame_tens.shape) == 4
                        frame = TensorUtils.to_numpy(frame_tens[env_i]) if is_single_obs else TensorUtils.to_numpy(frame_tens[env_i, -1])
    ...
    if hasattr(policy, 'save_stats'):
        video_path = video_writer.request.filename
        save_dir = os.path.dirname(os.path.dirname(video_path))
        task_name = os.path.basename(video_path).replace('.mp4', '')
        policy.save_stats(save_dir, task_name)
...
def run_rollout_from_demo(
    ...
    assert not batched, "run_rollout_from_demo not supported for batched envs yet"

    env.reset_to(initial_state) if initial_state is not None else env.reset()
    ob_dict = env.get_observation()
            ...
            if video_count % video_skip == 0:
                if batched:
                    ...
                    for env_i in range(len(env)):
                        ...
                        frame = TensorUtils.to_numpy(frame_tens[env_i])
                        frame = np.transpose(frame, (1, 2, 0))
...
def rollout_with_stats(
    ...
    num_episodes=None,
    render=False,
    output_dir=None,  # for save results
    video_dir=None,
    video_path=None,
    epoch=None,
    video_skip=5,
    ...
    iterator0 = LogUtils.custom_tqdm(total=len(horizon_list), desc="Envs", position=0)
    for env, horizon in zip(envs, horizon_list):
        ...
        if not verbose:
            iterator = LogUtils.custom_tqdm(iterator, total=len(iterator), desc="Num. Rollout Steps", position=1, leave=False)
        ...
        if video_path is not None:
            with jsonlines.open(os.path.join(output_dir, "results.jsonl"), mode='a') as f:
                f.write({"env": env_name, **all_rollout_logs[env_name]})
            print(f'results written to {os.path.join(output_dir, "results.jsonl")}')
...
def rollout_from_demo(
    ...
    data_logger=None,
    ds_format="robomimic",
    initial_statess=None,  # ! from_demo (n_envs, n_episodes, state)
):
    """
    A helper function used in the train loop to conduct evaluation rollouts per environment
    and summarize the results.
    ...
    iterator0 = LogUtils.custom_tqdm(total=len(horizon_list), desc="Envs", position=0)
    # for env, horizon in zip(envs, horizon_list):
    initial_statess = [None for _ in range(len(envs))] if initial_statess is None else initial_statess  # ! from_demo
    for env, horizon, initial_states in zip(envs, horizon_list, initial_statess):  # ! from_demo
        num_episodes = len(initial_states) if initial_states is not None else num_episodes  # ! from_demo
        if env is None:
            continue
        ...
        for ep_i in iterator:
            rollout_timestamp = time.time()
            rollout_info = run_rollout_from_demo(
                policy=policy,
                env=env,
                horizon=horizon,
                render=render,
                ...
                terminate_on_success=terminate_on_success,
                ds_format=ds_format,
                initial_state=None if initial_states is None else initial_states[ep_i],  # ! from_demo
            )
