...
class OctoInference:
    def __init__(
        ...
        if model is not None:
            ...
            self.tokenizer, self.tokenizer_kwargs = None, None
            self.model = OctoModel.load_pretrained(self.model_type, model_step)
    ...
    def step(self, image: np.ndarray, task_description: Optional[str] = None, *args, **kwargs) -> tuple[dict[str, np.ndarray], dict[str, np.ndarray]]:
        ...
        norm_raw_actions, embeddings = self.model.sample_actions(
            input_observation,
            self.task,
            rng=key,
        )
        # obs_heatmap = np.array(jax.numpy.abs(embeddings['obs_primary'].tokens[0, 0]).mean(-1).reshape(16, 16))
        # resize heatmap
        # import cv2
        # obs_heatmap = cv2.resize(obs_heatmap, (256, 256))
        # plt.imshow(image)
        # plt.imshow(obs_heatmap, alpha=0.3, cmap='jet')
        # plt.savefig('visualize/heatmap/{}.png'.format(kwargs['timestep']))
        # plt.close()
